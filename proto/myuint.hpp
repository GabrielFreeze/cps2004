...
//TODO: Make operators work with different templates
...
template <char16_t Size> class myuint {
    ...
    public:
        ...
        void setBit(uint64_t n, uint8_t x) {
            setBit(myuint<Size>(n),x);
        }

        template <char16_t U>
        void setBit(myuint<U> n, uint8_t x) {
            myuint<Size> a(0);
            a = n;
        ...
        constexpr void setBit(myuint<Size> n, uint8_t x) {
            ...
            if constexpr (Size > 128) {
                ...
                if (n < Size/2) {
                    ...
                    n.setBit(log(Size/2),0); //n.setBit(log(Size/2),0) == n - Size/2
                    a->setBit(n,x); 
                ...
                if (Size > 64) {
                    ...
                    memcpy(&a, data[0], Size/8);
                    if (x) a |=   1ULL << b;
                    else   a &= ~(1ULL << b);
                    memcpy(data[0], &a, Size/8);
                }
            }
        }
        ...
        constexpr inline bool operator < (const myuint& rhs) {
